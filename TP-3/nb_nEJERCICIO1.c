...
double **media;                     /* valores de las medias            */
double **desvsest;                  /* valores de desvios estandars     */
double **suma;                      /* para calcular el desvio estandar */
...
int define_matrix(){
  ...
  suma=(double **)calloc(N_Class,sizeof(double *));
  media=(double **)calloc(N_Class,sizeof(double *));
  desvsest=(double **)calloc(N_Class,sizeof(double *));

  for(i=0;i<N_Class;i++){
    sumatoria[i]=(double *)calloc(N_IN+1,sizeof(double));
	}
  for(i=0;i<N_Class;i++){
    suma[i]=(double *)calloc(N_IN,sizeof(double));
	}
  for(i=0;i<N_Class;i++){
    media[i]=(double *)calloc(N_IN,sizeof(double));
	}
  for(i=0;i<N_Class;i++){
    desvsest[i]=(double *)calloc(N_IN,sizeof(double));
	}
...
double prob(double x,int feature,int clase)  {
  ...
  /*IMPLEMENTAR*/
  double pro;
  pro = (1/(sqrt(2*PI)*desvsest[clase][feature]))*(exp((-0.5)*pow(((x-media[clase][feature])/desvsest[clase][feature]),2)));
  return pro;  
}
/* ------------------------------------------------------------------------------ */
...
int train(char *filename){
  ...
  for(cont2=0;cont2<N_IN+1;cont2++)
    for(cont=0;cont<N_Class;cont++) 
    {
      sumatoria[cont][cont2] = 0;
      suma[cont][cont2] = 0;
    }
  ...
  for(cont2=0;cont2<N_IN;cont2++)
    for(cont=0;cont<PR;cont++)
      if (data[cont][N_IN] == 0) {
        sumatoria[0][cont2] += data[cont][cont2];
        sumatoria[0][N_IN]++;
      }
      else {
        sumatoria[1][cont2] += data[cont][cont2];
        sumatoria[1][N_IN]++;
      }

  for(cont2=0;cont2<N_IN;cont2++)
    for(cont=0;cont<N_Class;cont++)
      media[cont][cont2] = sumatoria[cont][cont2] / (sumatoria[cont][N_IN] / N_IN);

  for(cont2=0;cont2<N_IN;cont2++)
    for(cont=0;cont<PR;cont++)
      if (data[cont][N_IN] == 0)
        suma[0][cont2] += pow((data[cont][cont2] - media[0][cont2]),2);
      else
        suma[1][cont2] += pow((data[cont][cont2] - media[1][cont2]),2);

  for(cont2=0;cont2<N_IN;cont2++)
    for(cont=0;cont<N_Class;cont++)
      desvsest[cont][cont2] = sqrt(suma[cont][cont2]/((sumatoria[cont][N_IN]/(N_IN))-1));
...
/*  for(cont=0;cont<N_IN;cont++)
    for(cont2=0;cont2<N_Class;cont2++) {
      printf("\nDesvio var %d clase %d: %f\n",cont, cont2,desvsest[cont2][cont]);
      printf("Media var %d clase %d: %f\n", cont, cont2,media[cont2][cont]);
  } 
*/
