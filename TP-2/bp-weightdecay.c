  ...
  for (patron=pat_ini; patron < pat_fin; patron ++) {
   ...
   /*nu tiene el numero del patron que se va a presentar*/
    ...
    for(i = 1; i <= N3; i++) {
      ...
      if(N3==1) if((x3[i]-0.5)*(S[nu][N1+i]-0.5)<0.) discrete_error+=1.0;    /*calcula error discreto para una sola salida - problema binario*/
    }
...
int train(char *filename){
  ...
  float eta,suma,gam,penterm1,pesos1,penterm2,pesos2;
  ...
  for (iter=1; iter<=ITER; iter++) {
    ...
    /* Termino penalizacion */
	  for( i = 1 ;i<= N3;i++) for( j = 0 ;j<= N2;j++){
		pesos2 = pesos2 + ((w2[i][j])*(w2[i][j]));
	  }
    ...
    penterm2 = gam * pesos2;
	  ...
	  for( i = 1 ;i<= N3;i++) for( j = 0 ;j<= N2;j++){
		dw2[i][j]= u * dw2[i][j] + eta * grad3[i] * x2[j] - eta * 2 * gam * w2[i][j];
		w2[i][j] += dw2[i][j];
	  }

    /* Termino penalizacion */
	  for( j = 1 ;j<= N2;j++) for( k = 0 ;k<= N1;k++){
	  pesos1 = pesos1 + ((w1[j][k])*(w1[j][k]));
	  }
    ...
    penterm1 = gam * pesos1;
	  ...
	  for( j = 1 ;j<= N2;j++) for( k = 0 ;k<= N1;k++){
		dw1[j][k]= u * dw1[j][k] + eta * grad2[j] * x1[k] - eta * 2 * gam * w1[j][k];
		w1[j][k] += dw1[j][k];
	  }
    ...
    if ((iter/NERROR)*NERROR == iter) {
      ...
      fprintf(ferror,"%f\t%f\t%f\t",disc_train,disc_valid,disc_test);
      fprintf(ferror,"%f\t",penterm1);     /* gurado el termino de penalizacion */
      fprintf(ferror,"%f\n",penterm2);     /* gurado el termino de penalizacion */
  ...
  for(k=0; k < PTEST ; k++){
    ...
    for(i=1; i<=N3; i++)	fprintf(fpredic,"%f\t",pred[k][i]);
    fprintf(fpredic,"\n");
  }
